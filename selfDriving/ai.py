...
class Network(nn.Module):
    ...
    def __init__(self,input_size,nb_actions):
        ...
        self.fc2 = nn.Linear(30,nb_actions) # hX
...
class Dqn():
    ...
    def select_action(self,state):
        ...
        acton = probs.multinomial()
        return acton.data(0,0)

    def learn(self, batch_state, batch_next_state, batch_reward, batch_action):
        output = self.model(batch_state).gather(1, batch_action.unsqeeze(1)).squeze(1)
        ...
        td_loss.backward(retain_vairables= True)
        self.optimizer.step()

    def update(self,reward,new_signal):
        new_state = torch.Tensor(new_signal).float().unsqueezw(0)
        self.memory.push(self.last_state, new_state, torch.LongTensor([int(self.last_action)]), torch.LongTensor([int(self.last_reward)]))
    ...
    def load(self):
        if os.path.isfile('last_brain.pth'):
            ...
            self.optimizer.load_state_dict(checkpoint['optimizerr'])
            print('done')
        else:
